 ...
 *  USA
 *
 *  $Id: m_kick.c 26094 2008-09-19 15:33:46Z androsyn $
 */
...
DECLARE_MODULE_AV1(kick, NULL, NULL, kick_clist, NULL, NULL, "$Revision: 26094 $");
...
m_kick(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	*buf = '\0';
	...
	if(msptr != NULL)
	{
		 ...
		 */
		if(IsServer(source_p))
			sendto_channel_local(chptr, ":%s KICK %s %s :%s",
		...
		sendto_server(client_p, chptr,
			      ":%s KICK %s %s :%s",
			      source_p->id, chptr->chname, source_p->id, comment);
